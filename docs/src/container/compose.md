...
By default, Perry's compose engine creates each container with a
`{md5(image)[0..8]}-{random_hex8}` derived name and **does not yet
register the service KEY (`db`, `api`, …) as a network alias**. So a
service config like:
...
api: {
  image: "myapp/api",
  environment: {
    // ❌ "db" doesn't resolve in DNS today
    DATABASE_URL: "postgres://user:pw@db:5432/app",
  },
}
```

will fail with `dial tcp: lookup db on 127.0.0.11:53: no such host` at
runtime. Until service-key network-alias support lands, **set
`container_name` explicitly on each service** and use those names in
...
This is the pattern the [Forgejo example](./production-patterns.md)
uses (`forgejo-db` and `forgejo-app`) — the chosen names are what
docker's embedded DNS resolves to within the user-defined bridge
network.
