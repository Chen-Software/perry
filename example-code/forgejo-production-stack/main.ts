 ...
 *
 * Run: perry compile crates/perry-container-compose/examples/forgejo/main.ts && ./main
 */
...
async function main() {
    ...
    const allRunning = statuses.every((s: any) => s.status === 'running' || s.status.includes('Up'));
    if (!allRunning) {
    ...
    console.log('PostgreSQL Health stdout:', postgresHealth.stdout);
