...
mod init_guard {
    ...
    pub extern "C" fn perry_init_guard_check_and_set(module_id: u64) -> i32 {
        ...
        if byte_idx >= GUARD_BYTES { return 0; }
...
pub extern "C" fn perry_runtime_widget_init() {
    ...
    std::panic::set_hook(Box::new(|info| {
        let msg = if let Some(s) = info.payload().downcast_ref::<&str>() { s.to_string() } else if let Some(s) = info.payload().downcast_ref::<String>() { s.clone() } else { "unknown panic".to_string() };
        let location = if let Some(loc) = info.location() { format!(" at {}:{}", loc.file(), loc.line()) } else { String::new() };
