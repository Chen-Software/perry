...
enum NativeArgKind {
    ...
    JsvalI64,
    /// Raw i64 value (fptosi f64 -> i64).
    I64Raw,
    /// Serialise arg to JSON string and pass as StringHeader pointer.
    Json,
}
...
const NATIVE_MODULE_TABLE: &[NativeModSig] = &[
    ...
    NativeModSig { module: "perry/container-compose", has_receiver: false, method: "up",
        class_filter: None,
        runtime: "js_container_compose_up", args: &[NA_JSON], ret: NR_PTR },
...
fn lower_native_module_dispatch(
    ...
    for (i, arg) in args.iter().enumerate() {
        ...
        match kind {
            ...
            NativeArgKind::I64Raw => {
                let blk = ctx.block();
                let i = blk.call(I64, "js_nanbox_get_pointer", &[(DOUBLE, &lowered)]);
                llvm_args.push((I64, i));
                arg_types.push(I64);
            }
            NativeArgKind::Json => {
                ...
                let zero_i32 = "0".to_string();
                let json_handle = blk.call(I64, "js_json_stringify", &[(DOUBLE, &lowered), (I32, &zero_i32)]);
                llvm_args.push((I64, json_handle));
                arg_types.push(I64);
            }
        }
    }
    ...
    for i in args.len()..sig.args.len() {
        match sig.args[i] {
            ...
            NativeArgKind::StrPtr | NativeArgKind::PtrI64 | NativeArgKind::JsvalI64 | NativeArgKind::I64Raw | NativeArgKind::Json => {
