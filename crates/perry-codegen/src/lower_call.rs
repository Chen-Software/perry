...
enum NativeArgKind {
    ...
    Bool,
    /// Integer: convert f64 to i32 via fptosi.
    I32,
    /// Integer: convert f64 to i64 via fptosi.
    I64,
    /// JSON: serialize JSValue to JSON string via js_json_stringify(v, 0).
    Json,
}
...
const NATIVE_MODULE_TABLE: &[NativeModSig] = &[
    ...
    NativeModSig { module: "perry/container", has_receiver: false, method: "exec",
        class_filter: None,
        runtime: "js_container_exec", args: &[NA_STR, NA_JSON, NA_JSON, NA_STR], ret: NR_PTR },
    ...
    NativeModSig { module: "perry/container-compose", has_receiver: false, method: "down",
        class_filter: None,
        runtime: "js_compose_down", args: &[NA_PTR, NA_BOOL], ret: NR_PTR },
    ...
    NativeModSig { module: "perry/container-compose", has_receiver: false, method: "logs",
        class_filter: None,
        runtime: "js_compose_logs", args: &[NA_PTR, NA_STR, NA_I32], ret: NR_PTR },
    NativeModSig { module: "perry/container-compose", has_receiver: false, method: "exec",
        class_filter: None,
        runtime: "js_compose_exec", args: &[NA_PTR, NA_STR, NA_JSON], ret: NR_PTR },
    ...
    NativeModSig { module: "perry/container-compose", has_receiver: true, method: "down",
        ...
        runtime: "js_compose_down", args: &[NA_BOOL], ret: NR_PTR },
    ...
    NativeModSig { module: "perry/workloads", has_receiver: true, method: "down",
        ...
        runtime: "js_workload_handle_down", args: &[NA_BOOL], ret: NR_PTR },
    ...
    NativeModSig { module: "perry/workloads", has_receiver: true, method: "logs",
        class_filter: Some("WorkloadHandle"),
        runtime: "js_workload_handle_logs", args: &[NA_STR, NA_I32], ret: NR_PTR },
...
fn lower_native_module_dispatch(
    ...
    for (i, arg) in args.iter().enumerate() {
        ...
        match kind {
            ...
            NativeArgKind::I32 => {
                let blk = ctx.block();
                let i32_v = blk.fptosi(DOUBLE, &lowered, I32);
                llvm_args.push((I32, i32_v));
                arg_types.push(I32);
            }
            NativeArgKind::I64 => {
                let blk = ctx.block();
                let i64_v = blk.fptosi(DOUBLE, &lowered, I64);
                llvm_args.push((I64, i64_v));
                arg_types.push(I64);
            }
            NativeArgKind::Json => {
                let blk = ctx.block();
                let zero = "0".to_string();
                let str_ptr = blk.call(PTR, "js_json_stringify", &[(DOUBLE, &lowered), (I32, &zero)]);
                let ptr_i64 = blk.ptrtoint(&str_ptr, I64);
                llvm_args.push((I64, ptr_i64));
                arg_types.push(I64);
            }
        }
    }
    ...
    for i in args.len()..sig.args.len() {
        match sig.args[i] {
            ...
            NativeArgKind::Bool | NativeArgKind::I32 => {
                llvm_args.push((I32, "0".to_string()));
                arg_types.push(I32);
            }
            NativeArgKind::I64 | NativeArgKind::Json => {
                llvm_args.push((I64, "0".to_string()));
                arg_types.push(I64);
            }
        }
