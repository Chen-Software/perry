...
console.log('🚀 Starting Forgejo stack...');
...
const allRunning = statuses.length === 2 && statuses.every((s) => s.status.toLowerCase().includes('up') || s.status.toLowerCase().includes('running'));
if (!allRunning) {
...
console.log(postgresHealth.stdout);
