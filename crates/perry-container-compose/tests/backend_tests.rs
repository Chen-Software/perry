...
mod tests {
    ...
    fn test_docker_run_args() {
        let driver = BackendDriver::Docker { bin: PathBuf::from("docker") };
        ...
        let args = OciCommandBuilder::run_args(&driver, &spec);
    ...
    #[test]
    fn test_apple_run_args_no_detach() {
        let driver = BackendDriver::AppleContainer { bin: PathBuf::from("container") };
        let spec = ContainerSpec {
            image: "alpine".into(),
            ..Default::default()
        };
    ...
    // Feature: perry-container | Layer: unit | Req: 1.2 | Property: -
    #[test]
    fn test_lima_subcommand_prefix() {
        let driver = BackendDriver::Lima { bin: PathBuf::from("limactl"), instance: "default".into() };
...
|-------------|-----------|-------|
| 1.1         | test_docker_run_args | unit |
| 1.2         | test_apple_run_args_no_detach | unit |
| 1.2         | test_lima_subcommand_prefix | unit |
| 1.6         | test_detect_backend_env_override | unit |
| 2.1         | test_docker_run_args | unit |
...
// Req 3.1, 3.2 — Data parsing tests are covered in OciBackend unit tests if needed.
