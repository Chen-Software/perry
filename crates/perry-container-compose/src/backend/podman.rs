//! Podman backend for Linux/Windows/other platforms.
...
async fn run_cmd(bin: &str, args: &[&str]) -> Result<std::process::Output> {
    let output = Command::new(bin).args(args).stdout(Stdio::piped()).stderr(Stdio::piped())
        .output().await.map_err(ComposeError::IoError)?;
    Ok(output)
}
...
struct InspectOutput { #[serde(rename = "State", default)] state: Option<InspectState> }
#[derive(Debug, Deserialize)]
struct InspectState { #[serde(rename = "Running", default)] running: Option<bool>, #[serde(rename = "Status", default)] status: Option<String> }
#[derive(Debug, Deserialize)]
struct ListEntry { #[serde(rename = "ID", default)] id: String, #[serde(rename = "Names", default)] names: Vec<String>, #[serde(rename = "Image", default)] image: String, #[serde(rename = "Status", default)] status: String, #[serde(rename = "Ports", default)] ports: Vec<String>, #[serde(rename = "Created", default)] created: String }
#[derive(Debug, Deserialize)]
struct ImageEntry { #[serde(rename = "Id", default)] id: String, #[serde(rename = "Repositories", default)] repository: String, #[serde(rename = "Tag", default)] tag: String, #[serde(rename = "Size", default)] size: u64, #[serde(rename = "Created", default)] created: String }
...
impl ContainerBackend for PodmanBackend {
    ...
    async fn check_available(&self) -> Result<()> {
        let output = Command::new(self.bin).arg("--version").stdout(Stdio::piped()).stderr(Stdio::piped())
            .output().await.map_err(ComposeError::IoError)?;
        if output.status.success() { Ok(()) } else {
            Err(ComposeError::BackendError { code: output.status.code().unwrap_or(-1), message: format!("'{}' not available: {}", self.bin, String::from_utf8_lossy(&output.stderr)) })
        }
    }
    ...
    async fn start(&self, id: &str) -> Result<()> { let output = run_cmd(self.bin, &["start", id]).await?; check_output(output)?; Ok(()) }
    async fn stop(&self, id: &str, _timeout: Option<u32>) -> Result<()> { let output = run_cmd(self.bin, &["stop", id]).await?; check_output(output)?; Ok(()) }
    async fn remove(&self, id: &str, force: bool) -> Result<()> {
        let mut args = vec!["rm"]; if force { args.push("-f"); } args.push(id);
        let output = run_cmd(self.bin, &args).await?; check_output(output)?; Ok(())
    }
    ...
    async fn inspect(&self, id: &str) -> Result<ContainerInfo> {
        ...
        if !output.status.success() {
            ...
            if is_not_found(&stderr) { return Err(ComposeError::NotFound(id.to_string())); }
        ...
        } else { serde_json::from_str::<InspectOutput>(json_str).ok() };
        match parsed {
            Some(info) => { let r = info.state.as_ref().and_then(|s| s.running).unwrap_or(false); Ok(ContainerInfo { id: id.to_string(), name: id.to_string(), image: String::new(), status: if r { "running" } else { "stopped" }.to_string(), ports: vec![], created: String::new() }) }
            None => Ok(ContainerInfo { id: id.to_string(), name: id.to_string(), image: String::new(), status: "stopped".to_string(), ports: vec![], created: String::new() }),
        }
    }
    ...
    async fn exec(&self, id: &str, cmd: &[String], env: Option<&HashMap<String, String>>, workdir: Option<&str>) -> Result<ContainerLogs> {
        ...
        args.push(id.into()); args.extend(cmd.iter().cloned());
        let arg_refs: Vec<&str> = args.iter().map(String::as_str).collect();
        let output = run_cmd(self.bin, &arg_refs).await?;
    ...
    async fn pull_image(&self, reference: &str) -> Result<()> { let output = run_cmd(self.bin, &["pull", reference]).await?; check_output(output)?; Ok(()) }
    ...
    async fn create_network(&self, name: &str, config: &ComposeNetwork) -> Result<()> {
        ...
        if let Some(lbls) = &config.labels { for (k, v) in lbls.to_map() { args.push("--label".into()); args.push(format!("{}={}", k, v)); } }
        args.push(name.into());
        let arg_refs: Vec<&str> = args.iter().map(String::as_str).collect();
        let output = run_cmd(self.bin, &arg_refs).await?; check_output(output)?; Ok(())
    }

    async fn remove_network(&self, name: &str) -> Result<()> {
        let output = run_cmd(self.bin, &["network", "rm", name]).await?;
        if !output.status.success() { let stderr = String::from_utf8_lossy(&output.stderr); if is_not_found(&stderr) { return Ok(()); } return Err(ComposeError::BackendError { code: output.status.code().unwrap_or(-1), message: stderr.to_string() }); }
        Ok(())
    }
...
impl Backend for PodmanBackend {
    ...
    async fn build(&self, context: &str, dockerfile: Option<&str>, tag: &str, args: Option<&HashMap<String, String>>, target: Option<&str>, network: Option<&str>) -> Result<()> {
        ...
        let mut ba_strs: Vec<String> = Vec::new(); if let Some(ba) = args { for (k, v) in ba { ba_strs.push(format!("{}={}", k, v)); } }
    ...
    async fn inspect(&self, name: &str) -> Result<ContainerStatus> {
        ...
        let stdout = String::from_utf8_lossy(&output.stdout);
        let parsed: Option<InspectOutput> = if stdout.trim().starts_with('[') {
            serde_json::from_str::<Vec<InspectOutput>>(stdout.trim()).ok().and_then(|v| v.into_iter().next())
        } else { serde_json::from_str::<InspectOutput>(stdout.trim()).ok() };
        match parsed {
    ...
    async fn logs(&self, name: &str, tail: Option<u32>, _follow: bool) -> Result<String> {
        ...
        args.push(name.to_owned()); let arg_refs: Vec<&str> = args.iter().map(String::as_str).collect();
        let output = run_cmd(self.bin, &arg_refs).await?; check_output(output)
    }
    ...
    async fn create_network(&self, name: &str, driver: Option<&str>, labels: Option<&HashMap<String, String>>) -> Result<()> {
        let config = ComposeNetwork { driver: driver.map(String::from), labels: labels.map(|l| { let mut m = indexmap::IndexMap::new(); for (k, v) in l { m.insert(k.clone(), Some(serde_yaml::Value::String(v.clone()))); } crate::types::ListOrDict::Dict(m) }), ..Default::default() };
    ...
    async fn create_volume(&self, name: &str, driver: Option<&str>, labels: Option<&HashMap<String, String>>) -> Result<()> {
        let config = ComposeVolume { driver: driver.map(String::from), labels: labels.map(|l| { let mut m = indexmap::IndexMap::new(); for (k, v) in l { m.insert(k.clone(), Some(serde_yaml::Value::String(v.clone()))); } crate::types::ListOrDict::Dict(m) }), ..Default::default() };
