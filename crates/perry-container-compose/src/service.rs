...
pub fn service_container_name(service: &crate::types::ComposeService, _service_name: &str) -> String {
    if let Some(name) = service.explicit_name() {
        return name.to_string();
    }

    let input = serde_yaml::to_string(service)
        .unwrap_or_else(|_| service.image.clone().unwrap_or_else(|| _service_name.to_string()));
    generate_name(&input)
}
