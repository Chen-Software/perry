...
pub trait CliProtocol: Send + Sync {
    ...
    fn parse_wait_output(&self, stdout: &str) -> Result<i32> {
        stdout.trim().parse().map_err(|_| ComposeError::BackendError {
            code: -1,
            message: format!("Invalid wait output: '{}'", stdout.trim()),
        })
    }
}

fn docker_run_flags(spec: &ContainerSpec, include_detach: bool) -> Vec<String> {
    let mut args = vec![if include_detach { "run" } else { "create" }.into()];
...
impl<P: CliProtocol + Send + Sync> ContainerBackend for CliBackend<P> {
    ...
    async fn start(&self, id: &str) -> Result<()> {
        self.exec_ok(self.protocol.start_args(id)).await.map(|_| ())
    }

    async fn stop(&self, id: &str, timeout: Option<u32>) -> Result<()> {
        self.exec_ok(self.protocol.stop_args(id, timeout)).await.map(|_| ())
    }

    async fn remove(&self, id: &str, force: bool) -> Result<()> {
        self.exec_ok(self.protocol.remove_args(id, force)).await.map(|_| ())
    }
    ...
    async fn exec(&self, id: &str, cmd: &[String], env: Option<&HashMap<String, String>>, workdir: Option<&str>) -> Result<ContainerLogs> {
        ...
        let mut full_args = self.protocol.subcommand_prefix().unwrap_or_default();
    ...
    async fn pull_image(&self, reference: &str) -> Result<()> {
        self.exec_ok(self.protocol.pull_image_args(reference)).await.map(|_| ())
    }
    ...
    async fn remove_image(&self, reference: &str, force: bool) -> Result<()> {
        self.exec_ok(self.protocol.remove_image_args(reference, force)).await.map(|_| ())
    }

    async fn create_network(&self, name: &str, config: &NetworkConfig) -> Result<()> {
        self.exec_ok(self.protocol.create_network_args(name, config)).await.map(|_| ())
    }

    async fn remove_network(&self, name: &str) -> Result<()> {
        self.exec_ok(self.protocol.remove_network_args(name)).await.map(|_| ())
    }

    async fn create_volume(&self, name: &str, config: &VolumeConfig) -> Result<()> {
        self.exec_ok(self.protocol.create_volume_args(name, config)).await.map(|_| ())
    }

    async fn remove_volume(&self, name: &str) -> Result<()> {
        self.exec_ok(self.protocol.remove_volume_args(name)).await.map(|_| ())
    }

    async fn wait(&self, id: &str) -> Result<i32> {
        let stdout = self.exec_ok(self.protocol.wait_args(id)).await?;
        self.protocol.parse_wait_output(&stdout)
    }
}
...
async fn probe_candidate(name: &str) -> std::result::Result<Arc<dyn ContainerBackend>, String> {
    ...
    match name {
        ...
        "colima" => {
            ...
            Ok(Arc::new(CliBackend::new(dbin, DockerProtocol)))
        }
        "lima" => {
