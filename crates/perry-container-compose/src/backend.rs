...
pub trait ContainerBackend: Send + Sync {
    ...
    fn build_args(&self, context: &str, dockerfile: Option<&str>, args: Option<&HashMap<String, String>>, tags: &[String]) -> Vec<String>;
...
pub trait CliProtocol: Send + Sync {
    ...
    fn build_args(&self, context: &str, dockerfile: Option<&str>, args: Option<&HashMap<String, String>>, tags: &[String]) -> Vec<String> {
        ...
        if let Some(df) = dockerfile { cli_args.extend(["-f".into(), df.into()]); }
        if let Some(a) = args { for (k, v) in a { cli_args.extend(["--build-arg".into(), format!("{}={}", k, v)]); } }
        for t in tags { cli_args.extend(["-t".into(), t.clone()]); }
        cli_args.push(context.into());
        cli_args
    }

    fn parse_container_id(&self, output: &str) -> String { output.trim().to_string() }
    ...
    fn parse_inspect_output(&self, output: &str) -> Result<ContainerInfo> {
        let v: Value = serde_json::from_str(output)?;
        let first = v.as_array().and_then(|a| a.first()).ok_or_else(|| ComposeError::NotFound("inspect output empty".into()))?;
        ...
        if let Some(arr) = v.as_array() {
            for i in arr {
                result.push(ImageInfo {
                    id: i["Id"].as_str().or(i["ID"].as_str()).unwrap_or("").to_string(),
                    repository: i["Repository"].as_str().unwrap_or("").to_string(),
                    tag: i["Tag"].as_str().unwrap_or("").to_string(),
                    size: i["Size"].as_u64().unwrap_or(0),
                    created: i["Created"].as_str().unwrap_or("").to_string(),
                });
            }
        }
        Ok(result)
    ...
    fn create_network_args(&self, name: &str, config: &NetworkConfig) -> Vec<String> {
        ...
        if config.internal.unwrap_or(false) { args.push("--internal".into()); }
        args.push(name.into());
        args
    }
...
impl CliProtocol for AppleContainerProtocol {
    ...
    fn build_args(&self, context: &str, _dockerfile: Option<&str>, _args: Option<&HashMap<String, String>>, tags: &[String]) -> Vec<String> {
        let mut cli_args = vec!["build".into(), "--cpus".into(), "2".into(), "--memory".into(), "2048MB".into(), "--arch".into(), "arm64".into(), "--os".into(), "linux".into()];
        for t in tags { cli_args.extend(["-t".into(), t.clone()]); }
        cli_args.push(context.into());
        cli_args
    }
    fn parse_container_info_json(&self, json: &Value) -> Result<ContainerInfo> {
        ...
        let name = config["labels"]["perry.container.name"].as_str()
        ...
        Ok(ContainerInfo { id, name, image, status, ports: Vec::new(), created: "".into() })
    }
}
...
impl<P: CliProtocol> ContainerBackend for CliBackend<P> {
    fn backend_name(&self) -> String { self.bin.file_name().unwrap_or_default().to_string_lossy().into() }
    fn build_args(&self, context: &str, dockerfile: Option<&str>, args: Option<&HashMap<String, String>>, tags: &[String]) -> Vec<String> {
        self.protocol.build_args(context, dockerfile, args, tags)
    }
    async fn exec_ok(&self, args: &[String]) -> Result<std::process::Output> {
    ...
    async fn run(&self, spec: &ContainerSpec) -> Result<ContainerHandle> {
        ...
        Ok(ContainerHandle { id: self.protocol.parse_container_id(&String::from_utf8_lossy(&output.stdout)), name: spec.name.clone() })
    }
    async fn create(&self, spec: &ContainerSpec) -> Result<ContainerHandle> {
    ...
    async fn list(&self, all: bool) -> Result<Vec<ContainerInfo>> {
        ...
        let output = self.exec_ok(&args).await?;
        self.protocol.parse_list_output(&String::from_utf8_lossy(&output.stdout))
    }
    async fn inspect(&self, id: &str) -> Result<ContainerInfo> {
        let output = self.exec_ok(&["inspect".into(), "--format".into(), "json".into(), id.into()]).await?;
        self.protocol.parse_inspect_output(&String::from_utf8_lossy(&output.stdout))
    }
    async fn logs(&self, id: &str, tail: Option<u32>) -> Result<ContainerLogs> {
    ...
    async fn list_images(&self) -> Result<Vec<ImageInfo>> {
        ...
        self.protocol.parse_list_images_output(&String::from_utf8_lossy(&output.stdout))
    }
    async fn remove_image(&self, reference: &str, force: bool) -> Result<()> {
...
pub async fn detect_backend() -> Result<Box<dyn ContainerBackend>> {
    ...
    for &name in candidates {
        match probe_candidate(name).await {
            Ok(be) => return Ok(be),
            Err(reason) => probed.push(BackendProbeResult { name: name.into(), available: false, reason }),
        }
    }
...
async fn probe_candidate(name: &str) -> std::result::Result<Box<dyn ContainerBackend>, String> {
    let check = match name {
        ...
        _ => return Err("unknown candidate".into()),
    };
    let bin = which::which(check.0).map_err(|_| format!("{} not found", check.0))?;
    ...
    if !matches!(probe_res, Ok(Ok(ref output)) if output.status.success()) {
         return Err("CLI check failed".into());
    }

    match name {
        "apple/container" => Ok(Box::new(AppleBackend::new(bin, AppleContainerProtocol))),
        "orbstack" => {
             ...
             if let Ok(Ok(output)) = res {
                 if let Ok(val) = serde_json::from_slice::<Value>(&output.stdout) {
                     if val.as_array().map_or(false, |a| a.iter().any(|m| m["Running"].as_bool() == Some(true))) {
                         return Ok(Box::new(DockerBackend::new(bin, DockerProtocol)));
                     }
                 }
             }
                 ...
                 for line in String::from_utf8_lossy(&output.stdout).lines() {
                     if let Ok(val) = serde_json::from_str::<Value>(line) {
                         if val["status"].as_str() == Some("Running") {
                             return Ok(Box::new(LimaBackend::new(bin, LimaProtocol { instance: "default".into() })));
                         }
                     }
                 }
             }
        ...
        _ => Ok(Box::new(DockerBackend::new(bin, DockerProtocol))),
    }
}
...
impl ContainerBackend for MockBackend {
    ...
    fn build_args(&self, _context: &str, _dockerfile: Option<&str>, _args: Option<&HashMap<String, String>>, _tags: &[String]) -> Vec<String> { vec![] }
    async fn exec_ok(&self, _args: &[String]) -> Result<std::process::Output> { Ok(std::process::Output { status: std::process::ExitStatus::default(), stdout: vec![], stderr: vec![] }) }
}
