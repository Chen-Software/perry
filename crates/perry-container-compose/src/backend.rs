...
impl CliProtocol for DockerProtocol {
    ...
    fn build_args(&self, spec: &crate::types::ComposeServiceBuild, image_name: &str) -> Vec<String> {
        ...
        if let Some(dockerfile) = &spec.dockerfile {
            args.extend(["-f".into(), dockerfile.clone()]);
        }
...
/// Executes CLI commands for a container runtime.
/// Generic over P: CliProtocol — zero vtable overhead.
#[derive(Debug)]
...
async fn probe_candidate(name: &str) -> std::result::Result<Arc<dyn ContainerBackend>, String> {
    let which_bin = |name: &str| -> std::result::Result<PathBuf, String> {
        which::which(name).map_err(|_| format!("{} binary not found on PATH", name))
    };
    ...
    match name {
        ...
        "orbstack" => {
            ...
            // Extra check for OrbStack: socket exists or version check
            let home = std::env::var("HOME").map(PathBuf::from).ok();
            let socket = home.map(|h| h.join(".orbstack/run/docker.sock"));
            let socket_exists = socket.map(|s| s.exists()).unwrap_or(false);
            if !socket_exists {
                Command::new(&bin).arg("--version").output().await.map_err(|_| "orbstack not responding")?;
            }
            Ok(Arc::new(CliBackend::new(bin, DockerProtocol)))
        }
        "colima" => {
            ...
            let dbin = which_bin("docker").map_err(|_| "docker cli not found (needed for colima)")?;
            Ok(Arc::new(CliBackend::new(dbin, DockerProtocol)))
        }
        "rancher-desktop" => {
            let bin = which_bin("nerdctl")?;
            // Rancher Desktop check: nerdctl + socket
            let home = std::env::var("HOME").map(PathBuf::from).ok();
            let socket = home.map(|h| h.join(".rd/run/containerd-shim.sock"));
            if let Some(s) = socket {
                if !s.exists() { return Err("rancher desktop socket not found".into()); }
            }
            Ok(Arc::new(CliBackend::new(bin, DockerProtocol)))
        }
        "lima" => {
