...
pub trait CliProtocol: Send + Sync {
    ...
    fn parse_list_output(&self, stdout: &str) -> Vec<ContainerInfo> {
        ...
        if let Ok(v) = serde_json::from_str::<Value>(stdout) {
             if let Some(arr) = v.as_array() {
                 for c in arr { if let Ok(info) = parse_container_info_from_json(c) { result.push(info); } }
             }
        }
        result
    }
    ...
    fn parse_list_images_output(&self, stdout: &str) -> Vec<ImageInfo> {
        ...
        if let Ok(v) = serde_json::from_str::<Value>(stdout) {
             if let Some(arr) = v.as_array() {
                 for i in arr { if let Ok(info) = parse_image_info_from_json(i) { result.push(info); } }
             }
        }
        result
    }
...
impl CliProtocol for AppleContainerProtocol {
    ...
    fn parse_inspect_output(&self, _id: &str, stdout: &str) -> Option<ContainerInfo> {
        if let Ok(v) = serde_json::from_str::<Value>(stdout) {
            if let Some(arr) = v.as_array() {
                if let Some(first) = arr.first() {
                    return parse_apple_container_info(first).ok();
                }
            }
        }
        None
    ...
    fn parse_list_output(&self, stdout: &str) -> Vec<ContainerInfo> {
        ...
        if let Ok(v) = serde_json::from_str::<Value>(stdout) {
             if let Some(arr) = v.as_array() {
                 for c in arr { if let Ok(info) = parse_apple_container_info(c) { result.push(info); } }
             }
        }
        result
    }
...
fn parse_apple_container_info(json: &Value) -> Result<ContainerInfo> {
    let config = &json["configuration"];
    let id = config["id"].as_str().unwrap_or("").to_string();
    let name = config["labels"]["perry.service"].as_str().or(config["id"].as_str()).unwrap_or("").to_string();
...
impl<P: CliProtocol + Send + Sync> ContainerBackend for CliBackend<P> {
    ...
    async fn logs(&self, id: &str, tail: Option<u32>) -> Result<ContainerLogs> {
        let mut args = Vec::new();
        if let Some(prefix) = self.protocol.subcommand_prefix() { args.extend(prefix); }
        args.extend(self.protocol.logs_args(id, tail));
        let output = Command::new(&self.bin).args(&args).output().await.map_err(ComposeError::IoError)?;
        Ok(ContainerLogs { stdout: String::from_utf8_lossy(&output.stdout).into(), stderr: String::from_utf8_lossy(&output.stderr).into() })
    }
    async fn exec(&self, id: &str, cmd: &[String], env: Option<&HashMap<String, String>>, workdir: Option<&str>) -> Result<ContainerLogs> {
        let mut args = Vec::new();
        if let Some(prefix) = self.protocol.subcommand_prefix() { args.extend(prefix); }
        args.extend(self.protocol.exec_args(id, cmd, env, workdir));
        let output = Command::new(&self.bin).args(&args).output().await.map_err(ComposeError::IoError)?;
        Ok(ContainerLogs { stdout: String::from_utf8_lossy(&output.stdout).into(), stderr: String::from_utf8_lossy(&output.stderr).into() })
    }
...
fn parse_image_info_from_json(json: &Value) -> Result<ImageInfo> {
    let id = json["Id"].as_str().or(json["ID"].as_str()).unwrap_or("").to_string();
    let repository = json["Repository"].as_str().unwrap_or("").to_string();
    let tag = json["Tag"].as_str().unwrap_or("").to_string();
    let size = json["Size"].as_u64().unwrap_or(0);
    let created = json["Created"].as_str().unwrap_or("").to_string();
    Ok(ImageInfo { id, repository, tag, size, created })
}
...
pub async fn detect_backend() -> std::result::Result<Box<dyn ContainerBackend + Send + Sync>, Vec<BackendProbeResult>> {
    if let Ok(name) = std::env::var("PERRY_CONTAINER_BACKEND") {
        match probe_candidate(&name).await {
            ...
            Err(reason) => return Err(vec![BackendProbeResult { name: name.clone(), available: false, reason }]),
        }
    }
    ...
    match name {
        ...
        "podman" => {
            ...
            if std::env::consts::OS == "macos" {
                 ...
                 } else { return Err("podman machine check failed".into()); }
            }
            Ok(Box::new(CliBackend::new(bin, DockerProtocol)))
        }
        "orbstack" => {
            let bin = which::which("orb").or_else(|_| which::which("docker")).map_err(|_| "orbstack/docker not found")?;
        ...
        "colima" => {
            ...
            if matches!(res, Ok(Ok(ref output)) if output.status.success() && String::from_utf8_lossy(&output.stdout).contains("running")) {
                let docker_bin = which::which("docker").map_err(|_| "docker cli not found")?;
                 ...
                 for line in String::from_utf8_lossy(&output.stdout).lines() {
                     if let Ok(val) = serde_json::from_str::<Value>(line) {
                         if val["status"].as_str() == Some("Running") {
                             let inst = val["name"].as_str().unwrap_or("default").to_string();
                             return Ok(Box::new(CliBackend::new(bin, LimaProtocol { instance: inst })));
                         }
                     }
                 }
             }
