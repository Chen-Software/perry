...
impl ComposeEngine {
    ...
    fn register(&self) -> ComposeHandle {
        ...
        COMPOSE_ENGINES.lock().unwrap().insert(
            stack_id,
                ...
                match res {
                    ...
                    Err(e) => {
                        ...
                        if !msg.contains("already exists") {
                            self.rollback(&started_containers, &created_networks, &created_volumes)
                                .await;
                            return Err(e);
                        }
                    }
            ...
            let container_spec = ContainerSpec {
                ...
                cmd: Some(match &svc.command {
                    ...
                    Some(serde_yaml::Value::Sequence(seq)) => {
                        seq.iter().map(|v| v.as_str().unwrap_or_default().to_string()).collect()
                    }
                    _ => vec![],
                }),
                entrypoint: None,
...
impl WorkloadGraphEngine {
    ...
    pub async fn run(
        &self,
        graph: crate::workload::WorkloadGraph,
        _opts: crate::workload::RunGraphOptions,
    ) -> Result<crate::workload::GraphHandle> {
        // Convert WorkloadGraph to ComposeSpec to reuse existing logic
        let mut compose_spec = ComposeSpec::default();
        compose_spec.name = Some(graph.name.clone());

        for (id, node) in &graph.nodes {
            let mut svc = crate::types::ComposeService::default();
            svc.image = node.image.clone();
            svc.container_name = Some(node.name.clone());
            svc.ports = Some(node.ports.iter().map(|p: &String| crate::types::PortSpec::Short(serde_yaml::Value::String(p.clone()))).collect());

            // Environment handling with references (simplified: literal only for compose conversion)
            let mut env_map = IndexMap::new();
            for (k, v) in &node.env {
                match v {
                    crate::workload::WorkloadEnvValue::Literal(s) => {
                        env_map.insert(k.clone(), Some(serde_yaml::Value::String(s.clone())));
                    }
                    crate::workload::WorkloadEnvValue::Ref(_) => {
                        // References will be resolved after graph start
                    }
                }
            }
            ...
            compose_spec.services.insert(id.clone(), svc);
        }

        let engine = ComposeEngine::new(compose_spec, graph.name.clone(), Arc::clone(&self.backend));
        let handle = engine.up(&[], true, false, false).await?;

        // After start, we would resolve refs here in a real implementation

        Ok(crate::workload::GraphHandle {
            graph_id: handle.stack_id,
