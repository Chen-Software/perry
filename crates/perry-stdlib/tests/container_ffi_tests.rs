...
async fn test_js_container_compose_ps_contract() {
    unsafe {
        let p = perry_stdlib::container::js_compose_ps(0.0);
        let res = await_promise_sync(p);
        assert!(res.is_err());
    }
}
...
async fn test_js_container_compose_logs_null() {
    unsafe {
        let p = perry_stdlib::container::js_compose_logs(0.0, null(), 10.0);
        let res = await_promise_sync(p);
        assert!(res.is_err());
    }
}
...
// Feature: perry-container | Layer: ffi-contract | Req: 11.7 | Property: -
#[tokio::test]
async fn test_js_compose_exec_null() {
    unsafe {
        let p = perry_stdlib::container::js_compose_exec(0.0, null(), null(), null());
        let res = await_promise_sync(p);
        assert!(res.is_err());
    }
}
...
// Feature: perry-container | Layer: ffi-contract | Req: 11.7 | Property: -
#[tokio::test]
async fn test_js_compose_down_contract() {
    unsafe {
        let p = perry_stdlib::container::js_compose_down(0.0, 1.0);
        let res = await_promise_sync(p);
        assert!(res.is_err());
    }
}
