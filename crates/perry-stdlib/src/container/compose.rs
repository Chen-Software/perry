...
pub async fn compose_up(spec: ComposeSpec) -> Result<(ComposeEngine, ComposeHandle), ContainerError> {
    let backend = get_global_backend().await;
    // TODO: support project name resolution in stdlib
    let engine = ComposeEngine::new(spec, "perry".into(), backend);
    let handle = engine.up(&[], true, false, false).await
        .map_err(|e| ContainerError::ServiceStartupFailed { service: "compose".into(), error: e.to_string() })?;
    Ok((engine, handle))
}
