...
use crate::common::handle::with_handle;
...
pub unsafe extern "C" fn js_container_stop(id_ptr: *const StringHeader, opts_ptr: *const StringHeader) -> *mut Promise {
    ...
    let opts: serde_json::Value = string_from_header(opts_ptr)
...
#[no_mangle]
pub unsafe extern "C" fn js_container_removeImage(ref_ptr: *const StringHeader, force: i64) -> *mut Promise {
    let promise = js_promise_new();
    let reference = match string_from_header(ref_ptr) {
        Some(s) => s,
        None => {
...
#[no_mangle]
pub unsafe extern "C" fn js_compose_logs(handle_id: i64, opts_ptr: *const StringHeader) -> *mut Promise {
...
pub unsafe extern "C" fn js_workload_handle_down(handle_id: f64, _opts_json_ptr: *const StringHeader) -> *mut Promise {
    js_container_compose_down(handle_id as i64, 0) // Shorthand
}

#[no_mangle]
pub unsafe extern "C" fn js_workload_handle_status(handle_id: f64) -> *mut Promise {
    js_container_compose_ps(handle_id as i64) // Shorthand
}
...
pub unsafe extern "C" fn js_workload_handle_logs(handle_id: f64, node_ptr: *const StringHeader, opts_json_ptr: *const StringHeader) -> *mut Promise {
    let mut opts_val = if opts_json_ptr.is_null() {
        serde_json::json!({})
    } else {
    ...
    if let Some(node) = string_from_header(node_ptr) {
        opts_val["service"] = serde_json::json!(node);
    }

    let opts_str = opts_val.to_string();
    let opts_ptr = perry_runtime::js_string_from_bytes(opts_str.as_ptr(), opts_str.len() as u32);
    js_container_compose_logs(handle_id as i64, opts_ptr)
}
...
pub unsafe extern "C" fn js_workload_handle_exec(handle_id: f64, node_ptr: *const StringHeader, cmd_json_ptr: *const StringHeader) -> *mut Promise {
    js_container_compose_exec(handle_id as i64, node_ptr, cmd_json_ptr, std::ptr::null())
}

#[no_mangle]
pub unsafe extern "C" fn js_workload_handle_ps(handle_id: f64) -> *mut Promise {
    js_container_compose_ps(handle_id as i64)
}
...
pub unsafe extern "C" fn js_container_compose_graph(handle_id: f64) -> *const StringHeader {
    let id = handle_id as i64;
    let json = with_handle::<Arc<ComposeEngine>, _, _>(id, |engine| {
