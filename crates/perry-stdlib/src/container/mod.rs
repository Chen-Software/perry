...
pub unsafe extern "C" fn js_workload_graph(name_ptr: *const StringHeader) -> f64 {
    ...
    WORKLOAD_GRAPH_HANDLES.get_or_init(DashMap::new).insert(id, graph);
    id as f64
}
...
pub unsafe extern "C" fn js_workload_node(
    ...
    let node: WorkloadNode = serde_json::from_str(&spec_json).unwrap_or_default();

    if let Some(mut graph_entry) = WORKLOAD_GRAPH_HANDLES.get_or_init(DashMap::new).get_mut(&(graph_handle as u64)) {
        graph_entry.nodes.insert(node_id.clone(), node.clone());
    }

    let id = NEXT_HANDLE_ID.fetch_add(1, Ordering::SeqCst);
    WORKLOAD_NODE_HANDLES.get_or_init(DashMap::new).insert(id, node);
    id as f64
}

#[no_mangle]
pub unsafe extern "C" fn js_workload_runGraph(graph_handle: f64, opts_ptr: *const StringHeader) -> *mut Promise {
    let promise = js_promise_new();
    let opts_json = string_from_header(opts_ptr).unwrap_or_default();

    crate::common::spawn_for_promise(promise as *mut u8, async move {
        let graph = {
            let handles = WORKLOAD_GRAPH_HANDLES.get_or_init(DashMap::new);
            handles.get(&(graph_handle as u64)).map(|g| g.clone())
        }.ok_or_else(|| compose_error_to_js(&ComposeError::NotFound("Workload graph not found".to_string())))?;

        let backend = get_global_backend_instance().await.map_err(|e| e.to_string())?;
        let engine = WorkloadGraphEngine::new(backend);
        let graph_json = serde_json::to_string(&graph).map_err(|e| compose_error_to_js(&ComposeError::JsonError(e)))?;
        let handle = engine.run(&graph_json, &opts_json).await.map_err(|e| compose_error_to_js(&e))?;
        Ok(handle.stack_id)
    });
    promise
}
...
#[no_mangle]
pub unsafe extern "C" fn js_workload_inspectGraph(graph_handle: f64) -> *mut Promise {
    let promise = js_promise_new();
    spawn_for_promise_deferred(promise as *mut u8, async move {
        let graph = {
            let handles = WORKLOAD_GRAPH_HANDLES.get_or_init(DashMap::new);
        ...
        let engine = WorkloadGraphEngine::new(backend);
        let graph_json = serde_json::to_string(&graph).map_err(|e| compose_error_to_js(&ComposeError::JsonError(e)))?;
        engine.status(&graph_json).await.map_err(|e| compose_error_to_js(&e))
    }, |status| {
        let json = serde_json::to_string(&status).unwrap_or_else(|_| "{}".to_string());
        let str_ptr = js_string_from_bytes(json.as_ptr(), json.len() as u32);
        JSValue::string_ptr(str_ptr).bits()
...
#[no_mangle]
pub unsafe extern "C" fn js_workload_handle_logs(handle_id: f64, node_ptr: *const StringHeader, tail: f64) -> *mut Promise {
...
pub unsafe extern "C" fn js_workload_handle_exec(
    ...
    js_container_compose_exec(handle_id, node_ptr, cmd_json_ptr, opts_json_ptr)
}
