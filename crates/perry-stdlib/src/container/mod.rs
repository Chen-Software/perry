...
pub use self::backend::get_backend_instance;
...
pub unsafe extern "C" fn js_container_run(spec_json: *const StringHeader) -> *mut Promise {
    ...
    crate::common::spawn_for_promise(promise as *mut u8, async move {
        let backend = get_backend_instance().await.map_err(|e| e.to_string())?;
...
#[no_mangle]
pub unsafe extern "C" fn js_workload_inspectGraph(handle_id_f64: f64) -> *mut Promise {
    js_workload_handle_status(handle_id_f64)
}

#[no_mangle]
pub unsafe extern "C" fn js_workload_handle_graph(handle_id_f64: f64) -> *const StringHeader {
    let handle_id = handle_id_f64 as u64;
...
pub unsafe extern "C" fn js_container_start(id_ptr: *const StringHeader) -> *mut Promise {
    ...
    crate::common::spawn_for_promise(promise as *mut u8, async move {
        let b = get_backend_instance().await.map_err(|e| e.to_string())?;
        b.start(&id).await.map(|_| 0u64).map_err(|e| e.to_string())
    });
    promise
}
...
pub unsafe extern "C" fn js_container_stop(id_ptr: *const StringHeader, timeout: f64) -> *mut Promise {
    ...
    crate::common::spawn_for_promise(promise as *mut u8, async move {
        let b = get_backend_instance().await.map_err(|e| e.to_string())?;
        b.stop(&id, t).await.map(|_| 0u64).map_err(|e| e.to_string())
    });
    promise
}
...
pub unsafe extern "C" fn js_container_remove(id_ptr: *const StringHeader, force: f64) -> *mut Promise {
    ...
    crate::common::spawn_for_promise(promise as *mut u8, async move {
        let b = get_backend_instance().await.map_err(|e| e.to_string())?;
        b.remove(&id, force != 0.0).await.map(|_| 0u64).map_err(|e| e.to_string())
    });
    promise
}
...
pub unsafe extern "C" fn js_container_exec(
    ...
    let env_str = string_from_header(env_json);
    let env: Option<std::collections::HashMap<String, String>> = env_str.and_then(|s| if s.is_empty() { None } else { serde_json::from_str(&s).ok() });
    let workdir = string_from_header(workdir_ptr);

    crate::common::spawn_for_promise(promise as *mut u8, async move {
        let b = get_backend_instance().await.map_err(|e| e.to_string())?;
        match b.exec(&id, &cmd, env.as_ref(), workdir.as_deref()).await {
            Ok(logs) => Ok(types::register_container_logs(logs)),
            Err(e) => Err(e.to_string()),
        }
    });
...
pub unsafe extern "C" fn js_container_pullImage(image_ptr: *const StringHeader) -> *mut Promise {
    ...
    crate::common::spawn_for_promise(promise as *mut u8, async move {
        let b = get_backend_instance().await.map_err(|e| e.to_string())?;
        b.pull_image(&image).await.map(|_| 0u64).map_err(|e| e.to_string())
    });
    promise
}
...
pub unsafe extern "C" fn js_container_removeImage(image_ptr: *const StringHeader, force: f64) -> *mut Promise {
    ...
    crate::common::spawn_for_promise(promise as *mut u8, async move {
        let b = get_backend_instance().await.map_err(|e| e.to_string())?;
        b.remove_image(&image, force != 0.0).await.map(|_| 0u64).map_err(|e| e.to_string())
    });
    promise
}
...
pub unsafe extern "C" fn js_container_getBackend() -> *const StringHeader {
    let b = tokio::runtime::Runtime::new().unwrap().block_on(get_backend_instance());
    match b {
        Ok(backend) => string_to_js(backend.backend_name()),
...
pub unsafe extern "C" fn js_workload_graph(name_ptr: *const StringHeader) -> f64 {
    ...
    perry_container_compose::workload::create_graph_builder(name) as f64
}

#[no_mangle]
pub unsafe extern "C" fn js_workload_node(graph_id_f64: f64, name_ptr: *const StringHeader, config_json: *const StringHeader) -> f64 {
    let graph_id = graph_id_f64 as u64;
...
#[no_mangle]
pub unsafe extern "C" fn js_workload_runGraph(graph_id_f64: f64, opts_json: *const StringHeader) -> *mut Promise {
    let graph_id = graph_id_f64 as u64;
    let promise = js_promise_new();
...
#[no_mangle]
pub unsafe extern "C" fn js_workload_handle_down(handle_id_f64: f64) -> *mut Promise {
    let handle_id = handle_id_f64 as u64;
    let promise = js_promise_new();
    crate::common::spawn_for_promise(promise as *mut u8, async move {
        if let Some(engine) = types::get_workload_engine(handle_id) {
...
#[no_mangle]
pub unsafe extern "C" fn js_workload_handle_status(handle_id_f64: f64) -> *mut Promise {
    let handle_id = handle_id_f64 as u64;
    let promise = js_promise_new();
    crate::common::spawn_for_promise(promise as *mut u8, async move {
        if let Some(engine) = types::get_workload_engine(handle_id) {
...
#[no_mangle]
pub unsafe extern "C" fn js_container_compose_down(handle_id_f64: f64, volumes: f64) -> *mut Promise {
    let handle_id = handle_id_f64 as u64;
    let promise = js_promise_new();
    crate::common::spawn_for_promise(promise as *mut u8, async move {
...
#[no_mangle]
pub unsafe extern "C" fn js_workload_handle_ps(handle_id_f64: f64) -> *mut Promise {
    let handle_id = handle_id_f64 as u64;
    let promise = js_promise_new();
    crate::common::spawn_for_promise(promise as *mut u8, async move {
        if let Some(engine) = types::get_workload_engine(handle_id) {
...
#[no_mangle]
pub unsafe extern "C" fn js_container_compose_ps(handle_id_f64: f64) -> *mut Promise {
    let handle_id = handle_id_f64 as u64;
    let promise = js_promise_new();
    crate::common::spawn_for_promise(promise as *mut u8, async move {
        if let Some(engine) = types::get_compose_engine(handle_id) {
...
pub unsafe extern "C" fn js_container_compose_exec(
    ...
    let env_str = string_from_header(env_json);
    let env: Option<std::collections::HashMap<String, String>> = env_str.and_then(|s| serde_json::from_str(&s).ok());

    crate::common::spawn_for_promise(promise as *mut u8, async move {
        if let Some(engine) = types::get_compose_engine(handle_id) {
            match engine.exec(&service, &cmd, env.as_ref(), None).await {
                Ok(res) => Ok(types::register_container_logs(types::ContainerLogs {
                    stdout: res.stdout,
                    stderr: res.stderr,
                })),
...
#[no_mangle]
pub unsafe extern "C" fn js_container_compose_config(handle_id_f64: f64) -> *mut Promise {
    let handle_id = handle_id_f64 as u64;
    let promise = js_promise_new();
    crate::common::spawn_for_promise(promise as *mut u8, async move {
        if let Some(engine) = types::get_compose_engine(handle_id) {
...
#[no_mangle]
pub unsafe extern "C" fn js_container_compose_start(handle_id_f64: f64, services_json: *const StringHeader) -> *mut Promise {
    let handle_id = handle_id_f64 as u64;
    let promise = js_promise_new();
...
#[no_mangle]
pub unsafe extern "C" fn js_container_compose_stop(handle_id_f64: f64, services_json: *const StringHeader) -> *mut Promise {
    let handle_id = handle_id_f64 as u64;
    let promise = js_promise_new();
    let services_str = string_from_header(services_json).unwrap_or_default();
    let services: Vec<String> = serde_json::from_str(&services_str).unwrap_or_default();
...
#[no_mangle]
pub unsafe extern "C" fn js_container_compose_restart(handle_id_f64: f64, services_json: *const StringHeader) -> *mut Promise {
    let handle_id = handle_id_f64 as u64;
    let promise = js_promise_new();
    let services_str = string_from_header(services_json).unwrap_or_default();
    let services: Vec<String> = serde_json::from_str(&services_str).unwrap_or_default();
...
pub extern "C" fn js_container_module_init() {
    let _ = tokio::runtime::Runtime::new().unwrap().block_on(get_backend_instance());
}

// ============ perry/compose Aliases ============

#[no_mangle]
pub unsafe extern "C" fn js_compose_up(spec_json: *const StringHeader) -> *mut Promise {
    js_container_compose_up(spec_json)
}

#[no_mangle]
pub unsafe extern "C" fn js_compose_down(handle_id_f64: f64, volumes: f64) -> *mut Promise {
    js_container_compose_down(handle_id_f64, volumes)
}

#[no_mangle]
pub unsafe extern "C" fn js_compose_ps(handle_id_f64: f64) -> *mut Promise {
    js_container_compose_ps(handle_id_f64)
}
...
pub unsafe extern "C" fn js_compose_logs(
    ...
    js_container_compose_logs(handle_id_f64, service_ptr, tail)
}
...
pub unsafe extern "C" fn js_compose_exec(
    ...
    js_container_compose_exec(handle_id_f64, service_ptr, cmd_ptr, env_json)
}

#[no_mangle]
pub unsafe extern "C" fn js_compose_config(handle_id_f64: f64) -> *mut Promise {
    js_container_compose_config(handle_id_f64)
}

#[no_mangle]
pub unsafe extern "C" fn js_compose_start(handle_id_f64: f64, services_json: *const StringHeader) -> *mut Promise {
    js_container_compose_start(handle_id_f64, services_json)
}

#[no_mangle]
pub unsafe extern "C" fn js_compose_stop(handle_id_f64: f64, services_json: *const StringHeader) -> *mut Promise {
    js_container_compose_stop(handle_id_f64, services_json)
}

#[no_mangle]
pub unsafe extern "C" fn js_compose_restart(handle_id_f64: f64, services_json: *const StringHeader) -> *mut Promise {
    js_container_compose_restart(handle_id_f64, services_json)
}
