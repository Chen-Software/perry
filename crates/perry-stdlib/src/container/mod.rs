...
#[no_mangle]
pub unsafe extern "C" fn js_compose_down(handle: f64, volumes: f64) -> *mut Promise {
    js_container_compose_down(handle, volumes)
}
...
///
/// FFI: `js_container_compose_down(handle: f64, volumes: f64) -> *mut Promise`
///
/// Args are `f64` (not `i64`/`i32`) because the codegen `NA_F64` arg-coercion
/// pushes them as LLVM `double`. The corresponding LL declare is
/// `declare i64 @js_container_compose_down(double, double)`, so Rust must
/// match that calling convention.
#[no_mangle]
pub unsafe extern "C" fn js_container_compose_down(handle: f64, volumes: f64) -> *mut Promise {
    let promise = js_promise_new();
    let handle_id = handle_id_from_f64(handle);
    let remove_volumes = volumes != 0.0;
