...
/// Public result of a signature verification attempt.
#[derive(Debug, Clone)]
pub struct VerificationResult {
    /// Whether the image passed verification.
    pub verified: bool,
...
/// for keyless verification, validates `CHAINGUARD_IDENTITY` and `CHAINGUARD_ISSUER`.
/// Returns a `VerificationResult` indicating success or failure.
pub async fn run_cosign_verify(
    reference: &str,
    _digest: &str,
